...
@interface WSDatePickerView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    ...
    NSString *_chooseYear;
    NSString *_chooseMonth;
    NSString *_chooseDay;
...
- (void)setisShouWeek:(BOOL)isShouWeek{
    _isShouWeek = isShouWeek;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    self.scrollToDate = [[NSDate date:dateStr WithFormat:@"yyyy年MM月dd日 HH时mm分"] dateWithFormatter:_dateFormatter];
...
- (NSInteger)DaysfromYear:(NSInteger)year andMonth:(NSInteger)month
{
    ...
    switch (num_month) {
        case 1:case 3:case 5:case 7:case 8:case 10:case 12:{
            [self setdayArray:31 year:num_year month:num_month];
            return 31;
        }
        case 4:case 6:case 9:case 11:{
            [self setdayArray:30 year:num_year month:num_month];
            return 30;
        }
        case 2:{
            if (isrunNian) {
                [self setdayArray:29 year:num_year month:num_month];
                return 29;
            }else{
                [self setdayArray:28 year:num_year month:num_month];
                return 28;
            }
        }
        default:
...
- (NSString *)getWeekWithYearMonth:(NSString *)yearMonth{
    
    NSDateFormatter* dateFormat = [[NSDateFormatter alloc]init];//实例化一个NSDateFormatter对象
    
    [dateFormat setDateFormat:@"yyyy-MM-dd"];//设定时间格式,要注意跟下面的dateString匹配，否则日起将无效
    
    NSDate*date =[dateFormat dateFromString:yearMonth];
...
- (NSString*)transformDate:(NSDate *)date formatterStr:(NSString *)formatterStr
{
