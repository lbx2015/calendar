...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
            [self animateIndicator:_indicators[i] Forward:NO complete:^{
                [self animateTitle:_titles[i] index:i show:NO complete:^{
    ...
    if (tapIndex == _currentSelectedMenudIndex && _show) {
        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex]  forward:NO index:(int)_currentSelectedMenudIndex complecte:^{
        ...
        [self animateIdicator:_indicators[tapIndex] background:_backGroundView tableView:_leftTableView title:_titles[tapIndex] forward:YES index:(int)_currentSelectedMenudIndex complecte:^{
            self.show = YES;
        }];
    }
}
...
- (void)animateTitle:(CATextLayer *)title index:(int)index show:(BOOL)show complete:(void(^)())complete {
    CGSize size = [self calculateTitleSizeWithString:title.string];
    CGFloat sizeWidth = (size.width < (self.frame.size.width / _numOfMenu) - 25) ? size.width : self.frame.size.width / _numOfMenu - 25;
    title.bounds = CGRectMake(0, 0, sizeWidth, size.height);
    if ([_indicators[index] isKindOfClass:[UIImageView class]]) {
        
        UIImageView *imageView = _indicators[index];
        
        CGRect imageViewFrame = imageView.frame;
        imageViewFrame.origin.x = CGRectGetMaxX(title.frame)+1;
...
- (void)animateIdicator:(id)indicator background:(UIView *)background tableView:(UITableView *)tableView title:(CATextLayer *)title forward:(BOOL)forward index:(int)index complecte:(void(^)())complete{
    if (self.indicatorAlignType == DOPIndicatorAlignTypeCloseToTitle) {
        [self layoutIndicator:indicator withTitle:title];
    }
    [self animateIndicator:indicator Forward:forward complete:^{
        [self animateTitle:title index:index show:forward complete:^{
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row column:_currentSelectedMenudIndex]> 0) {
        ...
        if (self.isClickHaveItemValid) {
            ...
            [self animateTitle:title index:(int)_currentSelectedMenudIndex show:YES complete:^{
                [_rightTableView reloadData];
            }];
        } else {
            [_rightTableView reloadData];
