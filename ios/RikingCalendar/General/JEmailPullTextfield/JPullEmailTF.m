...
//    CGFloat textW = self.frame.size.width;
    ...
    self.pullTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, textY+textH+1, kScreenWidth, 4*cellH) style:UITableViewStylePlain];
...
- (void)textFieldDidChanged {
    ...
    if (self.text.length>0) {
        ...
        if ([self.text containsString:@"@"]) {
            NSString *latterStr = [self.text substringFromIndex:[self.text rangeOfString:@"@"].location];
            if ([latterStr isEqualToString:@""]) {
                self.matchedSuffixArray = self.mailsuffixData;
            } else {
                NSString *ssss = [self.text substringFromIndex:[self.text rangeOfString:@"@"].location+1];
                if ([ssss containsString:@"@"]) {
                    
                    self.text = [self.text substringWithRange:NSMakeRange(0, [self.text rangeOfString:@"@"].location+1)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *formerStr = self.text;
    if ([self.text containsString:@"@"]) {
        formerStr = [self.text substringToIndex:[self.text rangeOfString:@"@"].location];
    }
