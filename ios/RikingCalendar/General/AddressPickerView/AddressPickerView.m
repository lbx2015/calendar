...
@interface AddressPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame selectAddress:(NSString *)selectAddress{
...
static CGFloat const ADDRESSHEIGHT = 200;
...
- (UIView *)titleBackgroundView{
    if (!_titleBackgroundView) {
        _titleBackgroundView = [[UIView alloc]initWithFrame:
                                CGRectMake(0,self.bounds.size.height-TITLEHEIGHT-ADDRESSHEIGHT, SELFSIZE.width, TITLEHEIGHT)];
...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setImage:[UIImage imageNamed:@"chooseTime_cancel"] forState:UIControlStateNormal];
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn setImage:[UIImage imageNamed:@"chooseTime_sure"] forState:UIControlStateNormal];
...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        _addressPickerView = [[UIPickerView alloc]initWithFrame:
                              CGRectMake(0, self.bounds.size.height-ADDRESSHEIGHT, self.bounds.size.width, ADDRESSHEIGHT)];
...
- (void)loadPickerView{
    self.backgroundColor = RGBA(0, 0, 0, 0); UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(hide)];
    tap.delegate = self;
...
- (void)setIsAutoOpenLast:(BOOL)isAutoOpenLast {
    ...
    if (self.selectAddress && ![Utils isBlankString:self.selectAddress]) {
        [self.addressPickerView selectRow:self.p inComponent:0 animated:NO];
        [self.addressPickerView selectRow:self.c inComponent:1 animated:NO];
        [self.addressPickerView selectRow:self.a inComponent:2 animated:NO];
...
- (void)loadAddressData{
    ...
    for (int i = 0 ;i < _provincesArr.count; i++) {
        ...
        if (self.selectAddress && [self.selectAddress containsString:_provincesArr[i]]) {
            self.p = i;
        }
    ...
    for (int k = 0; k<_pArr.count; k++) {
        ...
        for (int j = 0; j<p.cities.count; j++) {
            ...
            if (self.selectAddress && [self.selectAddress containsString:cityName]) {
                self.c = j;
            }
            
            
            for (int z = 0; z<city.areas.count; z++) {
                
                NSString *areaName = city.areas[z];
                
                if (self.selectAddress && [self.selectAddress containsString:areaName]) {
                    self.a = z;
                }
...
//    }];
    ...
    if (isShow) {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            selfkY = 0;
            self.frame = CGRectMake(0,selfkY, self.bounds.size.width,[UIScreen mainScreen].bounds.size.height);
...
{
    if( [touch.view isDescendantOfView:self.titleBackgroundView]) {
        return NO;
    }
    return YES;
}
