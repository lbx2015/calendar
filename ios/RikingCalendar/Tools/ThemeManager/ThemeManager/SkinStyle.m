...
//    return [self.path stringByAppendingPathComponent:[self resourceForName:name]];
    return [[self resourceForName:name] stringByAppendingPathComponent:name];
}
...
+ (UIColor *)colorValue:(NSString *)colorString {
    
    NSArray *rgb = [[colorString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] componentsSeparatedByString:@","];
    if (rgb.count != 4) {
        NSLog(@"[error] color style format should be [r,g,b,alpha]");
        return [UIColor blackColor];
    }
    
    return [UIColor colorWithRed:[rgb[0] floatValue]/255 green:[rgb[1] floatValue]/255 blue:[rgb[2] floatValue]/255 alpha:[rgb[3] floatValue]];;
}
...
{    
    ...
    if ([cString length] < 8){
        return [UIColor hex_colorWithHex:stringToConvert];
