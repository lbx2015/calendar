...
public class AppUserReportRelServer {
	...
	// if (day.getIsWork()==1) {
	// set =
	// reportSubmitCaliberService.findAllByFreeDatefromReportId(appUserComplete.getAppUserId(),period.getWeek(),period.getTen(),period.getMonth(),period.getSeason(),period.getHalfYear(),period.getYear(),
	// 1);
	// freeSet =
	// reportSubmitCaliberService.findAllByFreeDatefromReportId(appUserComplete.getAppUserId(),periods.getWeek(),periods.getTen(),periods.getMonth(),periods.getSeason(),periods.getHalfYear(),periods.getYear(),
	// 0);
	...
	// String value = sysDataservice.getDict("T_REPORT_LIST", "MODLE_TYPE",
	// queryReport.getModuleType()).getValu();
