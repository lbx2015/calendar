...
public class RCompleteRelServer {
	...
	public AppResp getAllReport(@RequestBody Map<String, Object> params) {
		...
		List<QureyResulte> completeRels = reportCompletedRelRepo.findNowReport(rCompletedRelParams.getUserId(),
				completeDate);
	...
	// reportId = reportSubmitCaliberRepo.findByWorkDatefromReportId(period.getWeek(),
	// period.getTen(),
	// period.getMonth(), period.getSeason(), period.getHalfYear(), period.getYear());
	// Set<String> reportIdAdd =
	// reportSubmitCaliberRepo.findByFreeDatefromReportId(periods.getWeek(),
