...
public interface QACommentRepo extends JpaRepository<QAComment, String>, JpaSpecificationExecutor<QAComment> {
	 ...
	 * @return
	 */
	@Query("select new QAComment(q.id,q.createdTime,q.modifiedTime,q.isAduit,q.userId,q.questionAnswerId,q.content,(select a.userName from AppUser a where q.userId = a.id and a.isDeleted=1),(select ap.photoUrl from AppUserDetail ap where q.userId = ap.id)) from QAComment q where q.questionAnswerId = ?1 and q.isAduit <> 2 and q.isDeleted=1 order by q.createdTime desc")
