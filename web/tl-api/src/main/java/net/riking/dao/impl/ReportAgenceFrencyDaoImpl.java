...
public class ReportAgenceFrencyDaoImpl implements ReportAgenceFrencyDao {
	...
	public Set<String> findALLAgence() {
		...
		String sql = "SELECT substring_index(t.VALU, '-',  1) agenceName FROM t_base_modelpropdict t WHERE t.TABLENAME='T_REPORT_LIST' AND t.FIELD = 'MODLE_TYPE' GROUP BY agenceName";
	...
	public List<BaseModelPropdict> findAgenceNameList(String value) {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				BaseModelPropdict baseModelPropdict = new BaseModelPropdict(rs.getString(1), rs.getString(2), rs.getString(3));
	...
	public List<ReportFrequency> findReportByModuleType(String moduleType) {
		...
		SessionImplementor session = entityManager.unwrap(SessionImplementor.class);
		Connection connection = session.connection();
		String sql = "SELECT t.id,t.report_name,t.report_title,r.is_complete,group_concat(c.frequency ORDER BY c.frequency ASC ) AS strFrequency FROM t_report_list t LEFT JOIN t_base_modelpropdict m ON t.module_type = m.ke LEFT JOIN t_report_submit_caliber c ON t.id = c.report_id LEFT JOIN t_appuser_report_rel r ON r.report_id=t.id WHERE t.module_type=? GROUP BY t.id";
		PreparedStatement pstmt = null;
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				ReportFrequency reportFrequency = new ReportFrequency(rs.getString(1), rs.getString(2), rs.getString(3),rs.getString(4),rs.getString(5));
	...
	public List<ReportFrequency> findReportListByName(String reportName) {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				ReportFrequency report = new ReportFrequency(rs.getString(1), rs.getString(2), rs.getString(3),"",rs.getString(4));
				list.add(report);
			}
		} catch (SQLException e) {
			e.printStackTrace();
